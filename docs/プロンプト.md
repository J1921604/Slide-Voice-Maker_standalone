...
【重要タスク】
ワークスペース全ファイル解析。
下記ファイルを入力した状態で初期画面でデフォルト表示しろ。
- input\AIドリブン開発・教育体制の構築.pdf
- input\原稿.csv

解析した結果に合致しない内容を削除し下記ファイルをブラッシュアップ更新しろ。
...
下記を含むバックエンド版の記載は関連ファイルから完全に削除しろ。
- 音声・画像生成、動画生成
- バックエンド依存
- ワンクリック実行（run.bat/run.ps1削除しろ）
- ローカルサーバー
...
全ドキュメント内リンク先をローカル（例：docs/完全仕様書.md）ではなくリポジトリ（例：https://github.com/J1921604/Slide-Voice-Maker_standalone/blob/main/docs/完全仕様書.md）に正確に修正しろ
下記リポジトリに変更しろ
https://github.com/J1921604/Slide-Voice-Maker_standalone
...
https://github.com/J1921604/Slide-Voice-Maker_standalone
https://j1921604.github.io/Slide-Voice-Maker_standalone/
